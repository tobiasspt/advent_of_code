...
for step in steps:
    ...
    if "-" in step:
        ...
        for lense in box_dict[boxnum]:
            if label in lense:
                is_in = True
                already_in_lense = lense
                break
        if not is_in:
            box_dict[boxnum].append([label, focal_length])
        else:
            index = box_dict[boxnum].index(already_in_lense)
            box_dict[boxnum].insert(index, [label, focal_length])
            box_dict[boxnum].remove(already_in_lense)
            

def calculate_total_focusing_power(box_dict: dict) -> int:
