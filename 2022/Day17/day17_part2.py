...
def move_new_rock(shape_number, gas_index, stopped_shapes, ymax, i, read_pattern=False):
    ...
    while True:        
        ...
        if read_pattern and gas_index == 0:
            pattern_list.append([shape_number, ymax, i])
...
# We need to find patterns
def simulate_falling(number, read_pattern=False):
    ...
    for i in range(number):
        gas_index, stopped_shapes, ymax = move_new_rock(shape_number, gas_index, stopped_shapes, ymax, i, read_pattern=read_pattern)
...
pattern_list = []
# Fill pattern list
_ = simulate_falling(10000, read_pattern=True)


import numpy as np
# patttern array -> shape_number, height, index
pattern_array = np.array([pattern_list]).squeeze()
pattern_one_shape = pattern_array[pattern_array[:,0] == pattern_array[-1,0], :]
pattern_diffrences = np.diff(pattern_one_shape, axis=0)

if np.all(pattern_diffrences[-1,1] == pattern_diffrences[1:,1]):
    delta_height = pattern_diffrences[-1,1]
    delta_number = pattern_diffrences[-1,2]
else:
    print('Might need to look for more sophisticated pattern in data.')

height_to_pattern_start = pattern_one_shape[0,1]
number_to_pattern_start = pattern_one_shape[0,2]


n_total = 1000000000000

number_after_pattern = (n_total-number_to_pattern_start)%delta_number
height_after_pattern = simulate_falling(number_to_pattern_start +  delta_number + number_after_pattern ) - simulate_falling(number_to_pattern_start +  delta_number )


total_height = (n_total - number_to_pattern_start - number_after_pattern)/delta_number*delta_height + height_to_pattern_start + height_after_pattern

print('Part2:', int(total_height))
