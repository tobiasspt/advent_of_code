...
rounds = 50 #ome estimated number for the extra empty space in my grove array
#should be higher to be on the save side
...
while True:
    ...
    grove_array_new, rotation_index = move_one_round(grove_array, rotation_index)
    
    assert number_elves ==  np.sum(np.sum(grove_array=='#'))
    
    if np.all(grove_array_new == grove_array):
        break
    grove_array = grove_array_new
